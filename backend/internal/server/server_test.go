...
func TestInvalidInputs(t *testing.T) {
	s, _ := setup(t)
	w := httptest.NewRecorder()
	s.router.ServeHTTP(w, httptest.NewRequest(http.MethodPost, "/members", bytes.NewReader([]byte(`{}`))))
	if w.Code != 400 {
		t.Fatalf("status %d", w.Code)
	}

	w2 := httptest.NewRecorder()
	s.router.ServeHTTP(w2, httptest.NewRequest(http.MethodPost, "/teams", bytes.NewReader([]byte(`{"name":""}`))))
	if w2.Code != 400 {
		t.Fatalf("status %d", w2.Code)
	}

	w3 := httptest.NewRecorder()
	s.router.ServeHTTP(w3, httptest.NewRequest(http.MethodPost, "/assign", bytes.NewReader([]byte(`{"memberId":0,"teamId":0}`))))
	if w3.Code != 400 {
		t.Fatalf("status %d", w3.Code)
	}

	w4 := httptest.NewRecorder()
	s.router.ServeHTTP(w4, httptest.NewRequest(http.MethodPost, "/feedback", bytes.NewReader([]byte(`{"targetType":"x","targetId":0,"content":"a"}`))))
	if w4.Code != 400 {
